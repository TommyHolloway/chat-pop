...
Deno.serve(async (req) => {
  ...
  widgetScript += '  let sessionId = "session-" + Date.now() + "-" + Math.random().toString(36).substr(2, 9);\n';
    ...
    widgetScript += '\n';
    widgetScript += '    // Detect Shopify order confirmation (thank you page)\n';
    widgetScript += '    if (window.Shopify?.checkout?.order_id || window.location.pathname.includes("/thank_you") || window.location.pathname.includes("/orders/")) {\n';
    widgetScript += '      const orderId = window.Shopify?.checkout?.order_id || new URLSearchParams(window.location.search).get("order_id");\n';
    widgetScript += '      const orderTotal = window.Shopify?.checkout?.total_price ? parseFloat(window.Shopify.checkout.total_price) : null;\n';
    widgetScript += '      const currency = window.Shopify?.checkout?.currency || window.Shopify?.currency?.active || "USD";\n';
    widgetScript += '      \n';
    widgetScript += '      if (orderId) {\n';
    widgetScript += '        console.log("ChatPop: Order detected - ", orderId);\n';
    widgetScript += '        \n';
    widgetScript += '        trackCartEvent("checkout_completed", null, orderTotal);\n';
    widgetScript += '        \n';
    widgetScript += '        if (orderTotal) {\n';
    widgetScript += '          fetch(supabaseUrl + "/functions/v1/track-conversion", {\n';
    widgetScript += '            method: "POST",\n';
    widgetScript += '            headers: { "Content-Type": "application/json", "apikey": supabaseKey },\n';
    widgetScript += '            body: JSON.stringify({\n';
    widgetScript += '              sessionId: sessionId,\n';
    widgetScript += '              agentId: agentId,\n';
    widgetScript += '              orderId: orderId.toString(),\n';
    widgetScript += '              orderTotal: orderTotal,\n';
    widgetScript += '              currency: currency,\n';
    widgetScript += '              productsPurchased: window.Shopify?.checkout?.line_items || []\n';
    widgetScript += '            })\n';
    widgetScript += '          }).then(() => console.log("ChatPop: Conversion tracked successfully")).catch(error => console.error("ChatPop: Conversion tracking error:", error));\n';
    widgetScript += '        }\n';
    widgetScript += '      }\n';
    widgetScript += '    }\n';
    widgetScript += '  }\n\n';
    ...
    widgetScript += '      (isMobile() ? \'<button onclick="window.ChatPopWidget.close()" style="position:absolute;top:20px;right:20px;background:rgba(0,0,0,0.3);border:none;color:white;width:40px;height:40px;border-radius:50%;font-size:24px;cursor:pointer;z-index:10;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(5px);">×</button>\' : \'\') +\n';
