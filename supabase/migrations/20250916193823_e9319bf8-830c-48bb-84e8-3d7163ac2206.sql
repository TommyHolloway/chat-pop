-- CRITICAL SECURITY FIX: Profiles Table PII Protection (Clean Start)
-- Addresses: Customer Email and Phone Data Could Be Stolen by Hackers

-- 1. Thoroughly drop ALL existing policies on profiles table
DO $$ 
DECLARE
...
-- 2. Create enhanced authentication validation function
...
-- 3. Create secure profile access validation function
...
-- 4. Create input validation function for profile data
...
CREATE POLICY "Secure profile access - own data only"
ON public.profiles
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "Secure profile creation - validated input"
ON public.profiles
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "Secure profile updates - validated modifications"
ON public.profiles
FOR UPDATE
TO authenticated
USING (
...
-- 6. Block profile deletion completely for data protection
CREATE POLICY "Block all profile deletion"
ON public.profiles
FOR DELETE
TO authenticated
USING (false);
