-- Security Enhancement Migration: Critical Database Security Fixes
...
-- 2. Add RLS Protection for Security Audit Summary
...
CREATE POLICY "System can insert security audit data"
ON public.security_audit_summary
FOR INSERT
WITH CHECK (true); -- System-level insertions only

-- 3. Enhanced Lead Data Validation - Update existing trigger function
...
-- 6. Create comprehensive security maintenance function
...
BEGIN
  ...
  INSERT INTO public.security_audit_summary (
    audit_date,
    total_operations,
    service_role_ops,
    security_events,
  ...
  SELECT 
    CURRENT_DATE,
    COUNT(*),
    COUNT(*) FILTER (WHERE action LIKE 'SERVICE_ROLE_%'),
    COUNT(*) FILTER (WHERE action LIKE '%SECURITY%'),
    COUNT(DISTINCT ip_address),
    COUNT(*) FILTER (WHERE action LIKE '%lead%'),
    COUNT(*) FILTER (WHERE action LIKE '%visitor%')
  FROM activity_logs
