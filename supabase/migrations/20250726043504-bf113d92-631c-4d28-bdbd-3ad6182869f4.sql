...
DROP FUNCTION IF EXISTS public.check_user_plan_limits(UUID, TEXT, UUID, BIGINT);
DROP FUNCTION IF EXISTS public.update_storage_usage(UUID, BIGINT);

-- Create function to check user plan limits with secure search path
...
-- Function to update storage usage when files are uploaded/deleted with secure search path
