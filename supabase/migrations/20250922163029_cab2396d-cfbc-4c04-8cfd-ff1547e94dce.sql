...
-- 1. Add missing RLS policies for security_audit_summary table
ALTER TABLE public.security_audit_summary ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins only can access security audit summary" 
...
WITH CHECK (
  ...
  -- Rate limiting
  enhanced_rate_limit_check('lead_creation', 3, 60, true) AND
  -- Validate agent exists and is active
  EXISTS (
    SELECT 1 FROM agents 
    WHERE id = lead_data_json->'agent_id'::text::uuid AND status = 'active'
  )
);
...
BEGIN
  ...
  health_report := jsonb_build_object(
    ...
    'health_status', CASE 
      WHEN tables_without_rls = 0 AND pii_access_violations = 0 AND suspicious_patterns = 0 THEN 'healthy'
      WHEN tables_without_rls > 0 OR pii_access_violations > 0 THEN 'critical'
